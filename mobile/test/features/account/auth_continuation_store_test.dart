...
void main() {
  group('AuthContinuationStore', () {
    ...
    test('save-onboarding continuation round-trips and expires fail-closed', () async {
      ...
      expect(
        await store.read(now: DateTime.utc(2026, 7, 23, 12, 5)),
        pending,
      );
      expect(
    ...
    test('corrupt continuation fails closed and is deleted', () async {
      ...
      expect(
        await store.read(now: DateTime.utc(2026, 7, 23, 12)),
        isNull,
      );
      expect(file.existsSync(), isFalse);
    });

    test('coordinator refreshes pending onboarding intent and clears it', () async {
      final store = AuthContinuationStore(directoryResolver: () async => tempDir);
