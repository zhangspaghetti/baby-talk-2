...
void main() {
  group('GrowthStatsService', () {
    ...
    group('detectMilestones', () {
      test('empty events returns empty list', () {
        final result = service.detectMilestones(
          events: [],
          knownActivityIds: {'a1'},
          knownSpaceIds: {'s1'},
        );
      ...
      test('first event triggers first_opening milestone', () {
        final events = [
          _event(
            eventKey: 'e1',
            spaceId: 's1',
        ...
        expect(result.any((m) => m.id == 'first_opening'), isTrue);
        expect(
          result.any((m) => m.id == 'activity_a1_started'),
          isTrue,
        );
      });

      test('imitation triggers first_imitated milestone', () {
        final events = [
          _event(
            eventKey: 'e1',
            spaceId: 's1',
        ...
        expect(
          result.any((m) => m.id == 'first_imitated_a1'),
          isTrue,
        );
      });
      ...
      test('milestones are sorted chronologically', () {
        final events = [
          ...
          _event(
            ...
            activityId: 'a2',
            phraseId: 'p2',
            clientTimestamp: DateTime(2026, 5, 29, 11, 0),
          ),
        ];

        final result = service.detectMilestones(
          events: events,
          knownActivityIds: {'a1', 'a2'},
          knownSpaceIds: {'s1'},
        );

        for (var i = 1; i < result.length; i++) {
          expect(
            result[i].achievedAt.isAfter(result[i - 1].achievedAt) ||
                result[i].achievedAt == result[i - 1].achievedAt,
            isTrue,
          );
        }
      });
