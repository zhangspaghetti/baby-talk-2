...
Future<void> main(List<String> args) async {
  ...
  if (!options.runSmoke) {
    stdout.writeln('✅ S03 caregiver invite proof pack 文件检查通过。');
    stdout.writeln(
      '如需验证 landing/open-app/download surface，可追加 --smoke --base-url=http://127.0.0.1:8080 --token=<invite-token>。',
    );
    return;
  }
  ...
  if (options.token == null) {
    _fail('执行 --smoke 时必须提供 --token，例如 invite_token_1234', 64);
  }
...
Future<void> _runSmoke(_SmokeRequest request) async {
  ...
  stdout.writeln('==> S03 caregiver invite smoke');
  ...
  try {
    ...
    final failureReasonHeader = response.headers.value('x-invite-failure-reason');
    ...
    if (request.expectResult != null && resultHeader != request.expectResult) {
      _fail(
        'smoke 失败：预期 X-Invite-Result=${request.expectResult}，实际为 ${resultHeader ?? '(missing)'}。',
        1,
      );
    }
    ...
    stdout.writeln('✅ S03 caregiver invite smoke 通过。');
  } on TimeoutException {
    _fail(
...
void _assertFilesExist(Iterable<String> paths) {
  ...
  stderr.writeln('S03 caregiver invite proof pack 缺少必要文件：');
...
String _userAgentForPlatform(String? platform) {
  return switch (platform) {
    'android' => 'Mozilla/5.0 (Linux; Android 14) BabyTalkInviteVerify/1.0',
    'ios' => 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) BabyTalkInviteVerify/1.0',
    _ => 'BabyTalkInviteVerify/1.0',
  };
}
...
String? _defaultLocationForRoute(_RouteKind route) {
  return switch (route) {
    ...
    _RouteKind.openApp => 'babytalk://invite/open',
    _RouteKind.download => '/download?source=caregiver_invite',
  };
}
...
enum _RouteKind {
  ...
  String pathForToken(String token) {
    return switch (this) {
      _RouteKind.landing => '/invite/$token',
      _RouteKind.openApp => '/invite/$token/open-app',
      _RouteKind.download => '/invite/$token/download',
    };
  }
}
...
const String _usage = '''用法：
  dart run tool/verify_s03_invite.dart [--smoke --base-url=<url> --token=<token> [其它参数]]

说明：
  默认只检查 S03 caregiver invite proof pack 的必需文件是否存在：
    - docs/runbooks/s03-caregiver-invite.md
    - backend/src/main/resources/sql/s03_caregiver_invite_queries.sql
    - backend/src/main/resources/db/migration/V7__create_caregiver_invite_tables.sql
    - backend/src/main/resources/db/migration/V8__align_caregiver_invite_public_audit.sql
  ...
  --smoke                           从仓库根对 public invite surface 发起一次 HTTP smoke
  --base-url=<url>                  必填（仅在 --smoke 时），例如 http://127.0.0.1:8080
  --token=<value>                   必填（仅在 --smoke 时），例如 invite_token_1234
  ...
  --expect-result=<value>           预期 X-Invite-Result；默认按 route 推导
                                    传 skip 或 - 可跳过该断言
  --expect-body=<text>              可选，要求响应体包含指定文案
  --expect-location-contains=<text> 可选，要求 Location 头包含指定文案
                                    默认 open-app=babytalk://invite/open
                                    默认 download=/download?source=caregiver_invite
                                    传 skip 或 - 可跳过该断言
...
redaction 红线：
  这个 wrapper 只围绕 /invite/{token}、/invite/{token}/open-app、/invite/{token}/download、
  caregiver_invite_events、household_shared_context 与 release_distribution_events(source=caregiver_invite)
  的 coarse-grained surface 做存在性与可达性验证；不要把 child/installation/session/account、
  原始 interaction payload、完整 user-agent 或内部 debug 文案混进命令输出或后续扩展中。
''';
