...
HDR-R4-001 option 1 is confirmed and REFACTOR-020 implements the core-only local sensitive data clearance orchestrator without destructive flow wiring. REFACTOR-021 through REFACTOR-030 migrated `AccountSession.freezed.dart`, `InteractionEventEntity.g.dart`, `ShareLinkDraft.freezed.dart`, `LocalMentorSuggestion.freezed.dart`, `MentorFactEvent.freezed.dart`, `PracticePhrase.freezed.dart`, `InteractionEventPayload.freezed.dart`, `PracticeContinuitySnapshot.freezed.dart`, `PracticeActivityCatalog.freezed.dart`, and `GardenGrowthSnapshot.freezed.dart` into `mobile/lib/generated/`, reducing remaining co-located generated outputs to 1 Freezed file and 0 Isar/source_gen `.g.dart` files. Production readiness remains blocked by coverage below the 80% target, missing critical UI coverage slice, incomplete real-store lifecycle wiring, absent backup/encryption proof, absent performance benchmarks, final generated-code migration/hard-gate work, target-platform/CI replay, and pending final human release confirmation. The next task should be explicitly selected: complete the final generated-code migration, raise or except coverage, design/approve real destructive lifecycle wiring with UX confirmation, create performance benchmarks, replay integration evidence on target CI/platforms, or plan a narrowly scoped approved follow-up migration. Yellow design/lint decisions should be resolved before broader cleanup or before converting report-only gates into hard CI failures.
