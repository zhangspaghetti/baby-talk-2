...
- Review remediation: working tree on `codex/m1-first-care-turn-onboarding` (uncommitted), including starter persistence, serialized transitions, completion recovery, account-exit single-flight, origin-driven account return, typed continuation recovery, and monotonic formatter-baseline fixes, 2026-07-25.
...
| Whole mobile suite | `flutter test --concurrency=1 --reporter compact` | Prior pass (677); post-fix clean-commit rerun pending | The pre-final-fix 2026-07-25 suite completed with `All tests passed!`. Current remediation has fresh focused coverage; the full local CI is still blocked by the intentionally dirty worktree. |
| Local repository CI | `bash ci/full-ci.sh` | Blocked at clean-worktree gate | Fresh 2026-07-25 run stops by design with `full-ci: worktree must be clean before local full CI starts`; it cannot be represented as green before committing/staging in a clean worktree. |
...
| --- | --- | --- |
| Pixel 9 Pro emulator | Pass — focused smoke | Fresh install completed onboarding → Care Turn → trace → local completion → Today; a post-completion force-stop/relaunch restored Today and the continuity phrase. |
...
- This remediation could not reset the emulator without deleting retained app data. Windows UI capture for the emulator also failed with `SetIsBorderRequired failed: 不支持此接口 (0x80004002)`, so no additional scripted Android interaction was attempted.
- The whole-repository Dart formatter still reports 69 historical files. `ci/mobile-format-changed.sh` now requires the baseline to exactly equal that debt and only shrink relative to the full-CI merge base; it must be exercised by `full-ci` on a clean commit before approval.
- Full local CI cannot pass until the patch is committed/staged in a clean worktree; `ci/full-ci.sh` correctly rejects the current dirty tree before running its stages.
