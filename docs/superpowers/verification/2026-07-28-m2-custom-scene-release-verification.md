...
Generated-audio contract and failure evidence remains local, not endpoint E2E. `GeneratedUtteranceAudioServiceTest` covers accepted-session owner/ACTIVE/approved-utterance enforcement, empty/oversize/wrong-MIME provider output rejection, and retryable provider timeout. `flutter test test/features/care_path/presentation/care_turn_surface_test.dart test/app/local_sensitive_data_clearance_registry_test.dart test/core/local_data_lifecycle/local_sensitive_data_clearance_orchestrator_test.dart test/features/practice/generated/generated_audio_api_test.dart test/features/care_path/presentation/care_audio_playback_controller_test.dart` passed 20 tests. They cover authenticated route validation, empty/wrong-MIME/oversize client rejection, UI error fallback restoring `听一下`, late-playback cancellation, logout policy clearing `generatedAudioMemory`, and real registry clearance removing an occupied generated-audio cache. These use fakes/mocks where stated; they do not prove a backend endpoint response.

Android output-layer evidence: `flutter test integration_test/m2_generated_audio_bytes_playback_test.dart -d emulator-5554 --reporter expanded` rebuilt, installed, and passed on recreated API35 emulator (`00:41 +1: All tests passed!`). Test loads six independent known MP3 assets into memory and invokes production `AudioplayersCareAudioOutput.playBytes`, which uses `BytesSource`; each completion is awaited. This proves six controlled byte payloads play on current Android without temporary files. They are not generated clips or backend responses: consent/session authentication, owner/ACTIVE lookup, endpoint retrieval, actual generated TTS, and generated TTS failure behavior remain unproven on device.
