...
public class FakeCustomSceneGenerationService implements CustomSceneGenerationService {
    ...
    private GeneratedPracticeContentCandidate successCandidate(CustomSceneGenerationRequest request) {
        var scene = normalizeMode(request.canonicalSceneText());
        ...
        if (scene.contains("diaper") || scene.contains("尿布") || scene.contains("换")) {
            return candidate("日常照护", "换尿布", "Diaper change", "边换边轻声说，等宝宝看过来再重复。", "Fresh diaper.", "干净尿布。");
        }
        if (scene.contains("sleep") || scene.contains("bed") || scene.contains("睡") || scene.contains("哄")) {
            return candidate("家庭节奏", "睡前安抚", "Bedtime care", "放慢声音，先说短句再抱抱。", "Sleepy baby.", "宝宝困了。");
        }
        ...
        if (scene.contains("potty") || scene.contains("toilet") || scene.contains("如厕")) {
            return candidate("日常照护", "如厕时间", "Potty time", "陪着宝宝，轻声说一遍。", "Potty time.", "如厕时间。");
        }
