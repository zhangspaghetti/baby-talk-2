...
class PracticeCatalogRepositoryTest extends AbstractIntegrationTest {
    ...
    void findActivityBySceneTagReturnsSeedActivity() {
        var activity = catalogRepository.findActivityBySceneTag("Bath time");
    ...
    void findActivityBySceneTagReturnsEmptyForMissingScene() {
        assertThat(catalogRepository.findActivityBySceneTag(TEST_PREFIX + "missing_scene"))
                .isEmpty();
    }
    ...
    void findActivityBySceneTagUsesStableFirstInsertedTieBreakForDuplicateSceneTags() {
        ...
        var firstActivityId = catalogRepository.insertActivity(
        ...
        catalogRepository.insertActivity(
        ...
        var activity = catalogRepository.findActivityBySceneTag(TEST_PREFIX + "duplicate_scene");
    ...
    void findPhrasesByActivityIdReturnsSeedPhrasesInStepOrder() {
        ...
        var phrases = catalogRepository.findPhrasesByActivityId(bathTimeId);

        assertThat(phrases)
                .extracting(PracticeCatalogRepository.CachedPhrase::slug)
                .containsExactly(
    ...
    void findPhrasesByActivityIdUsesStableIdTieBreakForDuplicateSteps() {
        ...
        var firstPhraseId = catalogRepository.insertPhrase(
        ...
        var secondPhraseId = catalogRepository.insertPhrase(
        ...
        var phrases = catalogRepository.findPhrasesByActivityId(activityId);

        assertThat(phrases)
                .extracting(PracticeCatalogRepository.CachedPhrase::id)
                .containsExactly(firstPhraseId, secondPhraseId);
    }

    @Test
    void findSpaceIdBySlugReturnsExistingSeedAndEmptyForMissingSlug() {
        assertThat(catalogRepository.findSpaceIdBySlug("daily_care")).isPresent();
        assertThat(catalogRepository.findSpaceIdBySlug(TEST_PREFIX + "missing_space")).isEmpty();
    }

    @Test
    void findAllSpaceSlugsReturnsSeedSpacesInSortOrderWithoutDuplicates() {
        var slugs = catalogRepository.findAllSpaceSlugs();
    ...
    void insertSpaceReturnsInsertedIdAndConflictFallsBackToExistingId() {
        ...
        var insertedId = catalogRepository.insertSpace(slug, "Original title");
        var duplicateId = catalogRepository.insertSpace(slug, "Changed title");

        assertThat(duplicateId).isEqualTo(insertedId);
        assertThat(catalogRepository.findSpaceIdBySlug(slug)).contains(insertedId);
    ...
    void insertActivityReturnsInsertedIdWritesLlmSourceAndConflictFallsBackToExistingId() {
        ...
        var insertedId = catalogRepository.insertActivity(
                slug,
                spaceId,
                "Original activity",
                "Original scene",
        ...
        var duplicateId = catalogRepository.insertActivity(
                slug,
                spaceId,
                "Changed activity",
                "Changed scene",
    ...
    void insertPhraseReturnsInsertedIdWritesLlmSourceAndConflictFallsBackToExistingId() {
        ...
        var insertedId = catalogRepository.insertPhrase(
                slug,
                activityId,
                1,
                "Original English.",
        ...
        var duplicateId = catalogRepository.insertPhrase(
                slug,
                activityId,
                2,
                "Changed English.",
    ...
    void generatedPersistenceBehaviorKeepsRowsReadableByLegacyLookupMethods() {
        ...
        var activity = catalogRepository.findActivityBySceneTag(TEST_PREFIX + "generated_scene");
        var phrases = catalogRepository.findPhrasesByActivityId(activityId);
        ...
        assertThat(phrases)
                ...
                .containsExactly(firstPhraseId, secondPhraseId);
        assertThat(phrases)
                .extracting(PracticeCatalogRepository.CachedPhrase::difficulty)
                .containsExactly("easy", "medium");
    }

    @Test
    void findSpacesReturnsPublicSpaceRowsInSortOrderWithoutDuplicates() {
        var spaces = catalogRepository.findSpaces("zh-CN", 10);

        assertThat(spaces)
                .extracting(PracticeCatalogRepository.PracticeSpaceRow::spaceId)
                .containsExactly("daily_care", "family_rhythm");
        assertThat(spaces)
                .extracting(PracticeCatalogRepository.PracticeSpaceRow::titleZh)
                .containsExactly("日常照护", "家庭节奏");
        assertThat(spaces)
                .extracting(PracticeCatalogRepository.PracticeSpaceRow::sortOrder)
                .containsExactly(1, 2);
        assertThat(spaces.get(0).descriptionZh()).contains("洗澡");
        assertThat(new HashSet<>(spaces.stream()
                .map(PracticeCatalogRepository.PracticeSpaceRow::spaceId)
                .toList())).hasSameSizeAs(spaces);
    }

    @Test
    void findSpacesClampsLowerLimitBoundaryToOne() {
        var spaces = catalogRepository.findSpaces("zh-CN", 0);
    ...
    void findSpacesClampsUpperLimitBoundaryToFifty() {
        for (int i = 0; i < 55; i++) {
            catalogRepository.insertSpace(TEST_PREFIX + "limit_space_%02d".formatted(i), "Limit space " + i);
        }

        var spaces = catalogRepository.findSpaces("zh-CN", 1000);
    ...
    void findActivitiesBySpaceReturnsSeedActivitiesInSortOrderWithoutDuplicates() {
        var activities = catalogRepository.findActivitiesBySpace("daily_care", "zh-CN", 10);

        assertThat(activities)
                .extracting(PracticeCatalogRepository.PracticeActivityRow::activityId)
                .containsExactly("bath_time", "diaper_change");
        assertThat(activities)
                .extracting(PracticeCatalogRepository.PracticeActivityRow::spaceId)
                .containsExactly("daily_care", "daily_care");
        assertThat(activities)
                .extracting(PracticeCatalogRepository.PracticeActivityRow::source)
        ...
        assertThat(new HashSet<>(activities.stream()
                .map(PracticeCatalogRepository.PracticeActivityRow::activityId)
                .toList())).hasSameSizeAs(activities);
    }

    @Test
    void findActivitiesBySpaceClampsLowerLimitBoundaryToOne() {
        var activities = catalogRepository.findActivitiesBySpace("daily_care", "zh-CN", -5);
    ...
    void findActivitiesBySpaceReturnsEmptyForMissingSpace() {
        assertThat(catalogRepository.findActivitiesBySpace(TEST_PREFIX + "missing_space", "zh-CN", 10))
                .isEmpty();
    }

    @Test
    void findStarterPhraseReturnsSeedStarterPhraseWithFullFields() {
        var phrase = catalogRepository.findStarterPhrase("bath_time", "zh-CN");
    ...
    void findStarterPhrasePrefersStarterDifficultyBeforeStepOrder() {
        ...
        var phrase = catalogRepository.findStarterPhrase(TEST_PREFIX + "starter_preference_activity", "zh-CN");
    ...
    void findNextPhraseReturnsNextHigherStepWithinSameActivity() {
        var phrase = catalogRepository.findNextPhrase("bath_time", "bath_time_warm_water");
    ...
    void findNextPhraseReturnsEmptyForLastInvalidOrMismatchedPhrase() {
        assertThat(catalogRepository.findNextPhrase("bath_time", "bath_time_all_clean"))
                .isEmpty();
        assertThat(catalogRepository.findNextPhrase("bath_time", "feeding_time_open_wide"))
                .isEmpty();
        assertThat(catalogRepository.findNextPhrase(TEST_PREFIX + "missing_activity", "bath_time_warm_water"))
                .isEmpty();
    }

    @Test
    void existsSpaceActivityPhraseValidatesTheFullSlugPath() {
        assertThat(catalogRepository.existsSpaceActivityPhrase(
                "daily_care",
                "bath_time",
                "bath_time_warm_water"
        )).isTrue();
    ...
    private long insertSpaceFixture(String suffix) {
        return catalogRepository.insertSpace(TEST_PREFIX + suffix, "Test " + suffix);
    }
