...
class PracticeAiVersionLockVerifierTest(unittest.TestCase):
    ...
    def test_write_then_verify_accepts_origin_develop_base_lock(self) -> None:
        base_lock_text = subprocess.check_output(
            [
                "git",
                "show",
                "origin/Develop:backend/app-api/src/main/resources/config/practice-ai/version-lock.yml",
            ],
            cwd=REPO_ROOT,
            text=True,
        )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            base_lock.write_text(base_lock_text, encoding="utf-8")
            ...
            self.assertEqual(
                ...
                set(generated_by_version) - {entry["version"] for entry in base_entries},
            )
