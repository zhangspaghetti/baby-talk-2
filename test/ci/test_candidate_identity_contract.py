...
class CandidateIdentityContractTest(unittest.TestCase):
    ...
    def test_candidate_migration_contract_tracks_current_v35(self) -> None:
        values = VALUES.read_text(encoding="utf-8")
        qa_values = QA_VALUES.read_text(encoding="utf-8")
        production_values = PRODUCTION_VALUES.read_text(encoding="utf-8")
        script = QA_BOOTSTRAP.read_text(encoding="utf-8")

        self.assertIn('requiredMigrationVersion: "35"', values)
        self.assertIn('requiredMigrationVersion: "35"', qa_values)
        self.assertIn('requiredMigrationVersion: "35"', production_values)
        self.assertIn('QA_REQUIRED_MIGRATION_VERSION="${QA_REQUIRED_MIGRATION_VERSION:-35}"', script)
